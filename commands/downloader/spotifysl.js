...
const handler = async (m, { conn, text, usedPrefix }) => {
    ...
    try {
        ...
        if (!isUrl) {
            if (!global.APIs || !global.APIs.delirius || !global.APIs.delirius.url) 
                throw new Error("La API de Delirius no está configurada.");
...
// ✅ Export compatible con cmd.run y universal
module.exports = {
    run: handler,
